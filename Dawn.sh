...
function install_and_configure() {
    ...
    sudo apt update
    sudo apt install -y git tmux python3-venv libgl1-mesa-glx libglib2.0-0 libsm6 libxext6 libxrender-dev
    ...
    tmux send-keys -t dawn "python3 -m pip install -r requirements.txt" C-m  # 安装依赖
...
function setup_grassnode() {
    ...
    tmux send-keys -t grass "python3 main.py" C-m  # 启动 main.py
...
function setup_Nodepay() {
    ...
    function install_python() {
        ...
        # 安装 Python 和系统依赖
        sudo apt install -y python3 python3-venv python3-dev python3-pip libopencv-dev python3-opencv build-essential libssl-dev libffi-dev libxml2-dev libxslt1-dev zlib1g-dev libjpeg-dev libpng-dev libtiff-dev libcurl4-openssl-dev libgl1-mesa-glx libglib2.0-0 libsm6 libxext6 libxrender-dev libcairo2-dev pkg-config gcc g++ make git tmux
    ...
    tmux send-keys -t Nodepay "python3 -m venv venv" C-m
    tmux send-keys -t Nodepay "source venv/bin/activate" C-m
    # 安装依赖
    tmux send-keys -t Nodepay "python3 -m pip install --upgrade pip" C-m
    tmux send-keys -t Nodepay "python3 -m pip install curl_cffi fake-useragent" C-m
    tmux send-keys -t Nodepay "python3 -m pip install -r requirements.txt" C-m
...
# 2024.12.29
# Source:
# https://raw.githubusercontent.com/sdohuajia/Dawn/refs/heads/main/Dawn.sh
############################################################
